...
module.exports = async (ctx) => {
	...
	try {
		...
		let posts = await knex
			.table("t_posts")
			.select("r_id", "r_ticket_id", "r_content", "r_created_at");

		let date = posts.map((o) => r_created_at);
		console.log(date);

		let tickets2 = await knex.table("t_tickets").select();

		const ticketID = tickets2.map((o) => o.r_id);

		const ticket_username = await knex.table("t_posts").join("");

		const lastestMessage = await knex.table("t_posts").select;
